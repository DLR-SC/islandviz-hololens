...
namespace HoloIslandVis.Sharing
{
    ...
    /// </summary>
    public class CustomMessages : HoloToolkit.Unity.Singleton<CustomMessages>
    {
        /// <summary>
        /// Message enum containing our information bytes to share.
        /// The first message type has to start with UserMessageIDStart
        /// so as not to conflict with HoloToolkit ianternal messages.
        /// </summary>
        ...
        public void SendPanelTransform(Vector3 position, Quaternion rotation)
        {
            if (serverConnection != null && serverConnection.IsConnected())
            {
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.SurfaceTransform);
        ...
        public void SendIslandTransform(string name, Vector3 position, Quaternion rotation)
        {
            if (serverConnection != null && serverConnection.IsConnected())
            {
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.IslandTransform);

                AppendTransform(msg, position, rotation);
                XString m = new XString(name);
                msg.Write(m);
        ...
        public void SendContainerTransform(Vector3 position, Quaternion rotation)
        {
            if (serverConnection != null && serverConnection.IsConnected())
            {
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.ContainerTransform);
