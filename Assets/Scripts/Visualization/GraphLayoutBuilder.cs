...
namespace HoloIslandVis.Visualization
{
    ...
    {
        private static GraphLayoutBuilder _instance;
        public static GraphLayoutBuilder Instance {
            get {
                if (_instance == null)
                    _instance = new GraphLayoutBuilder();
        ...
        public GraphLayoutBuilder()
        {
            RNG = new System.Random(0);
        }

        //Public method to construct the physical distribution of islands in a certain volume
        public void ConstructRndLayout(BidirectionalGraph<GraphVertex, GraphEdge> g, Vector3 min, Vector3 max, float minDis, int maxIt)
        {
            graph = g;
            computeRNDGraphLayoutThreaded(min, max, minDis, maxIt);
        }

        public void ConstructFDLayout(OSGiProject proj, float stepSize, int simulationSteps)
        {
            project = proj;
            graph = proj.DependencyGraph;
                ...
                foreach (GraphVertex thisVert in graph.Vertices)
                {
                    ...
                    */
                    ...
                    foreach (GraphVertex otherVert in graph.Vertices)
                    {
                        ...
                        float distanceToBounds = direction.magnitude - (thisVert.Island.getRadius() + otherVert.Island.getRadius());
        ...
        private void computeRNDGraphLayoutThreaded(Vector3 distrBoxBegin, Vector3 distrBoxEnd, float minDistance, float maxIterations)
        {
            ...
            foreach (GraphVertex vertex in graph.Vertices)
            {
                ...
                int iteration = 0;
                while (!checkOverlap(rndVec, vertex.Island, minDistance) && iteration <= maxIterations)
                {
        ...
        private Boolean checkOverlap(Vector3 newPosition, CartographicIsland newIsland, float minDistance)
        {
            ...
            foreach (GraphVertex existingVertex in graph.Vertices)
            {
                ...
                float existingRadius = existingVertex.Island.getRadius();
