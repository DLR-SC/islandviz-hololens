...
// SHADER PROGRAMS //
// Vertex Shader
UCLAGL_v2g UCLAGL_vert(appdata_base v, float3 refPos, float3 refNorm, float refRad)
{
	UCLAGL_v2g output;

	float3 vertWorld = mul(unity_ObjectToWorld, v.vertex).xyz;
	float3 vertRef = vertWorld - refPos;
	float distToPlane = dot(vertRef, refNorm);
	float3 vertProj = vertWorld - distToPlane * refNorm;
	float distProj = distance(refPos, vertProj);

	if (distToPlane < -0.005f)
		output.blend = 0.0f;
	else if (distProj < refRad * 2)
		output.blend = 1.0f;
	else
		output.blend = pow(refRad / distProj, 20);
