...
Shader "Custom/StandardClippable"
{
    Properties
    {
		...
		// Clipping Planes
		_PositionClipBottom("Position Clip Bottom", Vector) = (0.0, 0.0, 0.0, 1.0)
		_PositionClipFront("Position Clip Front", Vector) = (0.0, 0.0, 0.0, 1.0)
		_PositionClipBack("Position Clip Back", Vector) = (0.0, 0.0, 0.0, 1.0)
		_PositionClipRight("Position Clip Right", Vector) = (0.0, 0.0, 0.0, 1.0)
		_PositionClipLeft("Position Clip Left", Vector) = (0.0, 0.0, 0.0, 1.0)

		_NormalClipBottom("Normal Clip Bottom", Vector) = (0.0, 0.0, 0.0, 1.0)
		_NormalClipFront("Normal Clip Front", Vector) = (0.0, 0.0, 0.0, 1.0)
		_NormalClipBack("Normal Clip Back", Vector) = (0.0, 0.0, 0.0, 1.0)
		_NormalClipRight("Normal Clip Right", Vector) = (0.0, 0.0, 0.0, 1.0)
		_NormalClipLeft("Normal Clip Left", Vector) = (0.0, 0.0, 0.0, 1.0)

		_BlendThreshold("Blend Threshold", Float) = 0.05
    }
    ...
    SubShader
    {
        Pass
        {
			...
			float4 _PositionClipBottom;
			float4 _PositionClipFront;
			float4 _PositionClipBack;
			float4 _PositionClipRight;
			float4 _PositionClipLeft;

			float4 _NormalClipBottom;
			float4 _NormalClipFront;
			float4 _NormalClipBack;
			float4 _NormalClipRight;
			float4 _NormalClipLeft;
            ...
            v2f vert(appdata_t v)
            {
                ...
                o.worldPosition.xyzw = mul(unity_ObjectToWorld, v.vertex).xyzw;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
				...
				float distBottom = dot(i.worldPosition.xyz - _PositionClipBottom, _NormalClipBottom);
				float distFront = dot(i.worldPosition.xyz - _PositionClipFront, _NormalClipFront);
				float distBack = dot(i.worldPosition.xyz - _PositionClipBack, _NormalClipBack);
				float distLeft = dot(i.worldPosition.xyz - _PositionClipLeft, _NormalClipLeft);
				float distRight = dot(i.worldPosition.xyz - _PositionClipRight, _NormalClipRight);
				...
				if (distFront < 0.0) {
					blendRegion = distFront + _BlendThreshold;
					blend *= abs(blendRegion / _BlendThreshold);
					albedo.a = blend;
					clip(blendRegion);
				}

				if (distBack < 0.0) {
					blendRegion = distBack + _BlendThreshold;
					blend *= abs(blendRegion / _BlendThreshold);
					albedo.a = blend;
					clip(blendRegion);
				}
				...
				if (distLeft < 0.0) {
					blendRegion = distLeft + _BlendThreshold;
					blend *= abs(blendRegion / _BlendThreshold);
					albedo.a = blend;
					clip(blendRegion);
				}
				...
				if (distRight < 0.0) {
					blendRegion = distRight + _BlendThreshold;
					blend *= abs(blendRegion / _BlendThreshold);
					albedo.a = blend;
					clip(blendRegion);
				}
